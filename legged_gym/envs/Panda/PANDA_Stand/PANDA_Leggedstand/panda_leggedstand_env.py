"""Panda rear-leg stand environment."""
...
def get_euler_xyz_tensor(quat):
    ...
    return euler_xyz
class PandaLeggedstand(BaseTask):
    ...
    def update_cmd_action_latency_buffer(self):
        """Return the action delayed by a per-environment number of sim steps."""
        ...
        return self.cmd_action_latency_buffer[
            env_ids, :, self.cmd_action_latency_simstep
        ]
    ...
    def reset_idx(self, env_ids):
        ...
        # Do not carry delayed actions across episode boundaries.
        ...
        # Synchronize state after reset and initialize delayed observations with
        # the new state instead of stale data from the previous episode.
        ...
        q_now = (self.dof_pos[env_ids] - self.default_dof_pos) \
            * self.obs_scales.dof_pos
        dq_now = self.dof_vel[env_ids] * self.obs_scales.dof_vel
        motor_now = torch.cat((q_now, dq_now), dim=1)
        imu_now = torch.cat((
            self.base_ang_vel[env_ids] * self.obs_scales.ang_vel,
            self.projected_gravity[env_ids]
        ), dim=1)
        self.obs_motor_latency_buffer[env_ids] = motor_now.unsqueeze(-1)
        self.obs_imu_latency_buffer[env_ids] = imu_now.unsqueeze(-1)
    ...
    def update_obs_latency_buffer(self):
        """Store scaled motor and IMU observations at every simulation step."""
        ...
        self.obs_motor_latency_buffer[:, :, 0] = motor_now
        ...
        self.obs_imu_latency_buffer[:, :, 0] = imu_now
    ...
    def compute_observations(self):
        """
        45-D actor observation with motor/IMU latency.
        ...
        if self.cfg.domain_rand.add_obs_latency:
            ...
            obs_motor = self.obs_motor_latency_buffer[
                env_ids, :, self.obs_motor_latency_simstep
            ]
            obs_imu = self.obs_imu_latency_buffer[
                env_ids, :, self.obs_imu_latency_simstep
            ]
            actor_ang_vel = obs_imu[:, 0:3]
            actor_gravity = obs_imu[:, 3:6]
            actor_dof_pos = obs_motor[:, 0:self.num_actions]
            actor_dof_vel = obs_motor[:, self.num_actions:2 * self.num_actions]
        else:
            actor_ang_vel = self.base_ang_vel * self.obs_scales.ang_vel
            actor_gravity = self.projected_gravity
            actor_dof_pos = q
            actor_dof_vel = dq
    ...
    def _process_dof_props(self, props, env_id):
        ...
        for i in range(len(props)):
             # Panda's URDF does not provide joint dynamics values. Multiplying
             # Isaac Gym's zero defaults would leave both randomizations at 0.
             if self.cfg.domain_rand.randomize_joint_friction:
    ...
    def _init_buffers(self):
        ...
        self.cmd_action_latency_buffer = torch.zeros(
            self.num_envs, self.num_actions, max_action_delay + 1,
        ...
        if self.cfg.domain_rand.randomize_cmd_action_latency:
            ...
            self.cmd_action_latency_simstep = torch.full(
                (self.num_envs,), max_action_delay,
        ...
        self.obs_motor_latency_buffer[:] = motor_now.unsqueeze(-1)
        self.obs_imu_latency_buffer[:] = imu_now.unsqueeze(-1)
    ...
    def _reward_default_pos_reward(self):
        pose_error = torch.sum(
            torch.abs(self.dof_pos - self.descire_joint_pos),
            dim=1
        )
    ...
    def _reward_symmetric_joints(self):
        dof = self.dof_pos.view(self.num_envs, 4, 3).clone()
        ...
        # FL 与 FR
        front_err = torch.sum(
            torch.abs(dof[:, 0, :] - dof[:, 1, :]),
            dim=1
        )

        return front_err * (self.rew_hanstand > 0.78)
    ...
    def _reward_front_stance_width(self):
        feet_rel = (
            self.feet_contact_pos
            - self.root_states[:, None, 0:3]
        )

        quat = self.base_quat[:, None, :].repeat(1, 2, 1)
        ...
        left_y = feet_body[:, 0, 1]
        right_y = feet_body[:, 1, 1]

        width = torch.abs(left_y - right_y)
        center = 0.5 * (left_y + right_y)

        return (
            torch.square(width - self.cfg.rewards.front_stance_width_target)
            + torch.square(center)
        )        
