...
class PandaLeggedstandCfg( LeggedRobotCfg ):
    ...
    class init_state( LeggedRobotCfg.init_state ):
        ...
        descire_joint_angles = { # = target angles [rad] when action = 0.0
            ...
            'FL_thigh_joint': -0.3665,     # [rad]
            'RL_thigh_joint': 0.4887,#1.,   # [rad]
            'FR_thigh_joint': -0.3665,     # [rad]
            'RR_thigh_joint': 0.4887,#1.,   # [rad]

            'FL_calf_joint': -2.0769,   # [rad]
            'RL_calf_joint': -1.85,    # [rad]
            'FR_calf_joint': -2.0769,  # [rad]
            'RR_calf_joint': -1.85,    # [rad]
                }
    class control:
        control_type = "P"
    ...
    class asset:
        ...
        feet_name_reward=['RL_foot_Link', 'RR_foot_Link']
        contact_foot=['FL_foot_Link', 'FR_foot_Link']
        # Target body pitch: about 80 degrees from the normal horizontal pose.
        # projected_gravity = [sin(80 deg), 0, -cos(80 deg)]
        target_gravity=[0.98480775, 0.0, -0.17364818]
        threshold=5.0
    ...
    class rewards:
        class scales:
            ...
            handstand_feet_height_exp=2.0
            default_pos_reward=1.0
            front_stance_width = -2.0
        front_stance_width_target = 0.38    
        only_positive_rewards = False # if true negative total rewards are clipped at zero (avoids early termination problems)
        tracking_sigma = 0.25 # tracking reward = exp(-error^2/sigma)
        soft_dof_pos_limit = 0.9 # percentage of urdf limits, values above this limit are penalized
        soft_dof_vel_limit = 1.
        soft_torque_limit = 1.
        base_height_target = 0.58#0.25
...
class PandaLeggedstandCfgPPO(LeggedRobotCfgPPO):
    seed = 1
    ...
    class runner:
        ...
        experiment_name = 'panda_leggedstand'
        run_name = ''
        # load and resume
        resume = True
        load_run = -1 # -1 = last run
        checkpoint = -1 # -1 = last saved model
        resume_path = '/home/ub/panda/My_unitree_go2_gym/logs/panda_leggedstand/Aug25_11-48-09_' # updated from load_run and chkpt
